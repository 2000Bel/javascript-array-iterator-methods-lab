...
let isAdultPresent = devs.some((older) =>{
...
let isEveryone19OrOlder = devs.every((old) =>{
    return old.year <= 2006;
});
...
let commentById = comments.find((especific) =>{
   return especific.id ===  823423;
});
...
let idx = comments.findIndex((inx) =>{
    return inx.id === 123523;
});
